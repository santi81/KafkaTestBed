...
class KafkaProducerProtocol[K: Manifest, V: Manifest](props: java.util.HashMap[String, Object],
  ...
  def call(session: Session,
           ...
           valueSchema: Option[Schema] = None): Unit = {
    ...
    } else if (keySchema.nonEmpty && valueSchema.nonEmpty) {
      key = dataGenerator.generateKey(keySchema)
      value = dataGenerator.generateValue(valueSchema)
    } else {
