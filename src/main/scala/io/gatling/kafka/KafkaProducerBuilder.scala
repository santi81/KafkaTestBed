...
case class KafkaProducerBuilder[K, V]( tableName: String = "",
                                       keySchema: Option[Schema] = None,
                                      valueSchema: Option[Schema] = None) extends ActionBuilder {
  ...
  override def build(ctx: ScenarioContext, next: Action): Action = {
    new KafkaProducerAction(
      ...
      ctx.coreComponents.statsEngine, next, tableName, keySchema, valueSchema)
  }
}